...
static size_t generate_dhcpv4_l2(uint8_t *buffer)
{
                       ...
                       size_t max_size) // зачем max_size
{
    ...
    uint8_t *packet_ptr = mt->out_buf + sizeof(struct pcap_file_header);
    generate_dhcpv4_l2(packet_ptr);

    *out_buf = mt->out_buf;
    return PCAP_FULL_SIZE;
}
