...
static void dhcp_invalidate(struct connman_dhcp* dhcp, bool callback)
{
    ...
    //Выводит отладочное сообщение — печатает адрес памяти структуры dhcp и значение callback (0 или 1)
    if (!dhcp)
        return;

    __connman_6to4_remove(dhcp->ipconfig);//Удаляет 6to4 туннель — если был настроен туннель IPv6-over-IPv4 (6to4), отключает его (потому что DHCP больше не работает)

    if (!apply_dhcp_invalidate_on_network(dhcp))//проверяет можно ли удалить DHCP конфиг этого интерфейса. Если возвращает false (нельзя удалить), функция выходит без продолжения
        return;

    __connman_ipconfig_set_dhcp_address(dhcp->ipconfig,
                                        __connman_ipconfig_get_local(dhcp->ipconfig));//запоминает текущий локальный IP адрес как "последний DHCP адрес"
    DBG("last address %s",
        __connman_ipconfig_get_dhcp_address(dhcp->ipconfig));

    __connman_ipconfig_address_remove(dhcp->ipconfig);//Удаляет IP адрес с интерфейса — убирает текущий IP с eth0/wlan0

    __connman_ipconfig_set_local(dhcp->ipconfig, NULL);//Очищает локальный IP в конфиге
    __connman_ipconfig_set_broadcast(dhcp->ipconfig, NULL);//удаляет сохраненный broadcast адрес
    __connman_ipconfig_set_gateway(dhcp->ipconfig, NULL);//Очищает шлюз по умолчанию
    __connman_ipconfig_set_prefixlen(dhcp->ipconfig, 0);//Очищает длину префикса сети — удаляет маску подсети (была /24, становится /0)

    if (dhcp->callback && callback)//вызывает callback уведомления, если он true
        dhcp->callback(dhcp->ipconfig, dhcp->network,
                       false, dhcp->user_data);//Это уведомляет верхние слои connman: "эй, DHCP конфиг больше недоступен, обновите свои данные"
}
...
//Объявление функции-колбэка — вызывается когда DHCP сервер отказал в аренде
{
    struct connman_dhcp* dhcp = user_data;
    int err;
    ...
    DBG("No lease available ipv4ll %d client %p", ipv4ll_running,
        dhcp->ipv4ll_client);
    //Выводит отладочное сообщение — печатает состояние IPv4LL (1=работает/0=нет) и адрес памяти IPv4LL клиента для отладки.

    dhcp->timeout = g_timeout_add_seconds(RATE_LIMIT_INTERVAL,
                                          dhcp_retry_cb,
                                          dhcp);
    //Создает таймер повтора — через RATE_LIMIT_INTERVAL секунд (60) вызовет dhcp_retry_cb()
    if (ipv4ll_running) //Проверяет статус IPv4LL — если IPv4LL уже запущен (автоматическое назначение IP 169.254.x.x)
        return;

    err = ipv4ll_start_client(dhcp);
    //Запускает IPv4LL клиент — пытается включить автоматическое назначение IP когда DHCP недоступен
    if (err < 0)
    ...
    //инвалидирует текущий IP конфиг и уведомляет верхние слои о проблеме. Срабатывает если ipv4ll_running false
}
...
static int dhcp_initialize(struct connman_dhcp* dhcp)
{
    ...
    index = __connman_ipconfig_get_index(dhcp->ipconfig);
    //Получает номер сетевого интерфейса из конфига IP (например, 2 для eth0)

    dhcp_client = g_dhcp_client_new(G_DHCP_IPV4, index, &error);
    //Создает новый DHCPv4 клиент для сетевого интерфейса index
    if (error != G_DHCP_CLIENT_ERROR_NONE)
        return -EINVAL;

    if (getenv("CONNMAN_DHCP_DEBUG"))
    {
        //проверка установлена ли эта переменная окружения
        dhcp->dhcp_debug_prefix = g_strdup_printf("DHCP index %d",
                                                  index); //для всех отладочных сообщений задаётся строка префикса
        g_dhcp_client_set_debug(dhcp_client, dhcp_debug,
                                dhcp->dhcp_debug_prefix);
        //регистрирует функцию dhcp_debug как обработчик отладочных сообщений клиента с префиксом
    }

    g_dhcp_client_set_id(dhcp_client);
    //Устанавливает Client ID — GLib использует MAC-адрес интерфейса для идентификации на сервере DHCP.

    if (dhcp->network)
    {
        //если установлена сеть
        struct connman_service* service;
        const char* hostname;
        //Ищет сервис связанный с сетью — возвращает объект сервиса или NULL
        service = connman_service_lookup_from_network(dhcp->network);
        //Получает hostname из сервиса
        hostname = __connman_service_get_hostname(service);
        if (!hostname)
            hostname = connman_utsname_get_hostname();
        //Если hostname не установлен в сервисе, получает hostname системы

        if (hostname) //регистрирует hostname как DHCP опцию HOST_NAME (12) для отправки на сервер.
            g_dhcp_client_set_send(dhcp_client,
                                   G_DHCP_HOST_NAME, hostname);

        g_dhcp_client_set_request(dhcp_client, G_DHCP_HOST_NAME); //Запрашивает опцию HOST_NAME
        g_dhcp_client_set_request(dhcp_client, G_DHCP_DNS_SERVER); //Запрашивает DNS серверы (опция 6)
        g_dhcp_client_set_request(dhcp_client, G_DHCP_DOMAIN_NAME); //Запрашивает домен (опция 15)
        g_dhcp_client_set_request(dhcp_client, G_DHCP_NTP_SERVER); //Запрашивает NTP серверы
        g_dhcp_client_set_request(dhcp_client, 252); //Запрашивает приватную опцию 252
        g_dhcp_client_set_request(dhcp_client, G_DHCP_MTU); //Запрашивает MTU (опция 26)
    }

    g_dhcp_client_set_request(dhcp_client, G_DHCP_SUBNET); //Запрашивает маску подсети (опция 1)
    g_dhcp_client_set_request(dhcp_client, G_DHCP_ROUTER); //Запрашивает шлюз по умолчанию (опция 3)

    g_dhcp_client_register_event(dhcp_client,
                                 G_DHCP_CLIENT_EVENT_LEASE_AVAILABLE,
                                 lease_available_cb,
                                 dhcp);
    //Регистрирует обработчик события,  когда получена аренда IP, вызовет функцию lease_available_cb(dhcp).

    g_dhcp_client_register_event(dhcp_client,
                                 G_DHCP_CLIENT_EVENT_LEASE_LOST, lease_lost_cb,
                                 dhcp); //когда потеряна аренда IP, вызовет функцию lease_lost_cb(dhcp).

    g_dhcp_client_register_event(dhcp_client,
                                 G_DHCP_CLIENT_EVENT_NO_LEASE, no_lease_cb,
                                 dhcp); //когда нет доступных аренд (сервер отказал)
